...
namespace ATOMv0.Models
{
  public class DimFacilitiesController : Controller
  {
    ...
    {
      if (string.IsNullOrEmpty(dimFacility.SiteName))
      {
        ModelState.AddModelError("SiteName", "Please Enter Site Name");
      }
      if (dimFacility.KeyCountry == null)
      {
        ModelState.AddModelError("KeyCountry", "Please select Country");
      }
      if (ModelState.IsValid)
      {
        dimFacility.IsActive = 1;
